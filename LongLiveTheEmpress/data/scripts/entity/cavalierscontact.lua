...
function CavaliersContact.initialize()
    ...
    self.Log(_MethodName, "Initialize Cavaliers Contact Script - Player should be hailed immediately")
...
function CavaliersContact.startTalk()
    ...
    local d0mid = {"We're glad to see you here"}
    if empress then
        table.insert(d0mid, "The empress sends her regards")
    else
        table.insert(d0mid, "The emperor sends his regards")
    end
    local missionconfirms = {"We'll contact you with the details.", "Here's the mission.", "Here are the details.", "Uploading mission data."}
    local goodbyes = {"Goodbye!", "Farewell!", "See you later!", "Until then!", "Be careful out there!"}
    ...
    --If the player sided with the emperor, we throw them a cursed bone.
    if not empress then
...
function CavaliersContact.warpOut()
    --No penalty for rejecting contact with them. They understand.
    if onClient() then
...
function CavaliersContact.escortShipment()
    local _MethodName = "Escort Shipment"
    if onClient() then
...
function CavaliersContact.destroyRaiders()
    local _MethodName = "Ambush Raiders"
    if onClient() then
        self.Log(_MethodName, "Invoking on Server")
...
function CavaliersContact.destroyResistance()
    local _MethodName = "Destroy Resistance"
    if onClient() then
        self.Log(_MethodName, "Invoking on Server")
...
function CavaliersContact.destroyXsotan()
    local _MethodName = "Destroy Xsotan"
    if onClient() then
        self.Log(_MethodName, "Invoking on Server")
...
function CavaliersContact.destroyOutpost()
    local _MethodName = "Destroy Outpost"
    if onClient() then
        self.Log(_MethodName, "Invoking on Server")
...
--ORDER FROM CHAOS (STORY 3)
function CavaliersContact.orderfromChaos()
    local _MethodName = "Order from Chaos"
    if onClient() then
        self.Log(_MethodName, "Invoking on Server")
