...
function Distributor.onDestroyedSector(idx, lastDamageInflictor)
    for _, distributor in pairs ({ Sector():getEntitiesByScript("distributor.lua") }) do
        distributor:invokeFunction("distributor.lua", "clearCurrentTarget")
    end
end
            ...
            if (entity.type == EntityType.Ship or entity.type == EntityType.Station) then
                if entity.index ~= self.data.currentTarget.index then
                    ...
                    if dist <= self.data.blastRadius then
                        ...
                        ESCCWeaponScriptUtil.inflictDamageToTarget(entity, self.data.damagePool * self.data.blastDamageMultiplier, _entity.index)
                    ...
                    if self.data.damagePrimaryTarget then
                        ...
                        ESCCWeaponScriptUtil.inflictDamageToTarget(entity, self.data.damagePool * self.data.primaryTargetDamageMultiplier, _entity.index)
                    end
                end
            end
        end
...
function Distributor.clearCurrentTarget()
    local methodName = "Clear Current Target"
